...
import { Mic, Square, X, Loader2, Volume2, Settings2 } from "lucide-react";
...
// Níveis de resposta — slider 1 a 5
...
function loadConfig(): VoiceConfig {
  try {
    ...
    if (s) return { ...{ voice: "nova", speed: "normal", responseLevel: 3, autoLoop: true }, ...JSON.parse(s) };
  } catch {}
  return { voice: "nova", speed: "normal", responseLevel: 3, autoLoop: true };
}
    ...
    <div className="fixed bottom-16 right-3 z-50 w-72 rounded-2xl shadow-2xl border border-white/10 bg-[#141c0d]/97 backdrop-blur-md overflow-hidden">
      ...
      {showSettings && (
        <div className="px-3 py-3 border-b border-white/10 bg-white/3 flex flex-col gap-3">
          ...
          {/* Tamanho da resposta — slider manual */}
          <div>
            ...
            <p className="text-[10px] text-gray-600 mt-1 text-center italic leading-snug">
              {currentLevel.prompt}
            </p>
          </div>
